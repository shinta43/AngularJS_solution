  ...
  function ($parse, $timeout, $compile, $q, UploadExif) {
    ...
    function resize(files, attr, scope) {
      ...
      if (resizeVal instanceof Function) {
        var defer = $q.defer();
        resizeVal(files).then(
          function (p) {
            resizeWithParams(p, files, attr, scope).then(
              function (r) {
                defer.resolve(r);
              },
              function (e) {
    ...
    upload.updateModel = function (ngModel, attr, scope, fileChange, files, evt, noDelay) {
      ...
      function resizeAndUpdate() {
        ...
        resize(validateAfterResize ? allNewFiles : valids, attr, scope).then(
          function () {
...
ngFileUpload.service("UploadValidate", [
  ...
  function (UploadDataUrl, $q, $timeout) {
    ...
    upload.validate = function (files, prevLength, ngModel, attr, scope) {
      ...
      promises.push(
        upload.happyPromise(
          validateAsync("maxDuration", "duration.max", /audio|video/, this.mediaDuration, function (d, val) {
      ...
      promises.push(
        upload.happyPromise(
          validateAsync("minDuration", "duration.min", /audio|video/, this.mediaDuration, function (d, val) {
            return d >= upload.translateScalars(val);
          })
        )
      );
  ...
  function (UploadValidate, $q) {
    ...
    upload.resize = function (file, width, height, quality, type, ratio, centerCrop, resizeIf, restoreExif) {
      ...
      upload.dataUrl(file, true).then(
        function (url) {
          resize(url, width, height, quality, type || file.type, ratio, centerCrop, resizeIf).then(
            function (dataUrl) {
